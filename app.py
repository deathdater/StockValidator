...
path = input("Enter the path of folder containing \"data.csv\" file for Nifty 50:")
try:
    with open(path + "/data.csv", newline='') as f:
        reader = csv.reader(f)
        stock_array = []
        # number_of_records = len(list(f))
        # print(number_of_records)
        # if f.readable() is True:
        for row in reader:
            # print(f.read())
            for i in row:  ## Intialize single Stock
